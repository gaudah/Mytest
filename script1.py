...
os.system("sudo dpkg -i INSTALL/Psycopg2/*.deb");
...
os.system("sudo -u postgres psql -c 'create user %s with superuser;'" %(username));
os.system("sudo -u postgres psql -c 'create database %s;'" %(dbname));
#os.system("sudo -u %s psql -c 'alter user %s with password 'm';'" %(username,username));
#alter user minu with password 'm';
